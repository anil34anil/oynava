...
export default function ProfilePage() {
  ...
  const TR = useAutoTr([
    "Profili düzenle", "Profili paylaş", "Kopyalandı", "Giriş Yap / Kayıt Ol",
    "İstatistikler", "Üye gün sayısı", "gün", "Beğenilen oyunlar", "Son oynananlar", "Jeton", "Çıkış Yap",
    "Beğenilen Oyunlar", "Ayarlar",
    "Henüz favori oyunun yok. Oyun kartlarındaki ♥ ile favorilerine ekle!", "Oyunları keşfet →",
    "Kullanıcı adı", "Kaydet", "Avatarın", "Mağaza →", "Henüz avatar yok.", "Mağazadan", "Rozetlerin",
    "Henüz rozet yok.", "Profilin şu an bu cihazda saklanıyor. Cihazlar arası senkron için giriş yap.",
  ]);
      ...
      <div className="overflow-hidden rounded-3xl border border-line bg-card">
        ...
        <div className="relative flex flex-col gap-4 px-5 pb-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex gap-2 pb-1">
            {user ? (
              <>
                <button onClick={() => setTab("ayarlar")} className="btn-ghost py-2 text-xs">✎ {TR[0]}</button>
                <button onClick={share} className="btn-ghost py-2 text-xs">↗ {shared ? TR[2] : TR[1]}</button>
              </>
            ) : (
              <button onClick={openLogin} className="btn-primary py-2 text-xs">{TR[3]}</button>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-[260px_1fr]">
        ...
        <aside className="space-y-4">
          <div className="card-base p-5">
            <h2 className="font-display text-lg font-bold text-ink">{TR[4]}</h2>
            <ul className="mt-3 space-y-3 text-sm">
              <li className="flex items-center gap-3">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-white/[0.06]">📅</span>
                <span>
                  <span className="block text-slate-500">{TR[5]}</span>
                  <b className="text-ink">{memberDays === null ? "—" : `${memberDays} ${TR[6]}`}</b>
                </span>
              </li>
              <li className="flex items-center gap-3">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-white/[0.06]">👍</span>
                <span>
                  <span className="block text-slate-500">{TR[7]}</span>
              ...
              <li className="flex items-center gap-3">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-white/[0.06]">🕒</span>
                <span>
                  <span className="block text-slate-500">{TR[8]}</span>
              ...
              <li className="flex items-center gap-3">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-white/[0.06]">🪙</span>
                <span>
                  <span className="block text-slate-500">{TR[9]}</span>
          ...
          {user && (
            <button onClick={logout} className="btn-ghost w-full py-2 text-xs">{TR[10]}</button>
          )}
        </aside>
          ...
          {tab === "begenilen" ? (
            <GameListFromIds
              ...
              empty={
                <div className="card-base p-8 text-center text-slate-500">
                  {TR[13]}
                  <div className="mt-3">
                    <Link href={href("/oyunlar")} className="text-neon hover:underline">{TR[14]}</Link>
                  </div>
                </div>
              }
            />
            ...
            <div className="space-y-5">
              ...
              <section className="card-base p-5">
                <label className="block text-xs uppercase tracking-wider text-slate-500">{TR[15]}</label>
              ...
              <section className="card-base p-5">
                <div className="mb-3 flex items-center justify-between">
                  <h2 className="font-display text-lg font-bold text-ink">{TR[17]}</h2>
                  <Link href={href("/magaza")} className="text-sm font-semibold text-neon hover:underline">{TR[18]}</Link>
                </div>
                <div className="flex flex-wrap gap-3">
                  ...
                  {ownedAvatars.length === 0 && (
                    <p className="self-center text-sm text-slate-500">
                      {TR[19]} <Link href={href("/magaza")} className="text-neon">{TR[20]}</Link>
                    </p>
                  )}
                </div>
              </section>
              ...
              {/* Rozetler */}
              <section className="card-base p-5">
                <h2 className="mb-3 font-display text-lg font-bold text-ink">{TR[21]}</h2>
                <div className="flex flex-wrap gap-3 text-2xl">
                  {ownedBadges.length > 0
                    ...
                    : <p className="text-sm text-slate-500">{TR[22]}</p>}
                </div>
              </section>

              {!user && (
                <p className="text-center text-xs text-slate-600">{TR[23]}</p>
              )}
            </div>
          )}
        </div>
